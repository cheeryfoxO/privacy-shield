...
const FingerprintCollector = {
  ...
  async collectAll() {
    ...
    results.push(await this.collectCanvasHash());
  ...
  // ============================================================
  // 7. Canvas 哈希（优先用页面采集值，popup 作为备用）
  // ============================================================
  async collectCanvasHash() {
    let hash = null;
    let fromPage = false;

    // 尝试读取页面 content script 采集的真实指纹
    try {
      const result = await chrome.storage.local.get(['canvasFingerprint:self']);
      if (result['canvasFingerprint:self']) {
        hash = result['canvasFingerprint:self'];
        fromPage = true;
      }
    } catch (e) {}
    ...
    if (!hash) {
      return {
        ...
        level: 'common',
        entropyBits: 0,
        detail: [{ label: '状态', value: 'Canvas 指纹采集失败' }]
      };
    }

    const level = fromPage ? 'uncommon' : 'uncommon';
    const entropyBits = 5;
